...
function initApp() {
  ...
  // Step 5 Crown Champion Card Dynamic Hydration
  if (fullData.pyramid_top && fullData.pyramid_top.length > 0) {
    const champ = fullData.pyramid_top[0];
    const champTitle = document.getElementById("champ-title");
    if (champTitle && champ.Strategy_Note) champTitle.textContent = champ.Strategy_Note;
    const champScore = document.getElementById("champ-score");
    if (champScore) champScore.textContent = (champ.Composite_Score ?? 96.4).toFixed(1);
    const champRet = document.getElementById("champ-return");
    if (champRet) champRet.textContent = `${(champ.Total_Return_Pct ?? 70.60) >= 0 ? '+' : ''}${(champ.Total_Return_Pct ?? 70.60).toFixed(2)}%`;
    const champAlpha = document.getElementById("champ-alpha");
    const mktMove = fullData.benchmark?.total_return ?? -18.68;
    if (champAlpha) champAlpha.textContent = `+${((champ.Total_Return_Pct ?? 70.60) - mktMove).toFixed(2)}%`;
    const champMdd = document.getElementById("champ-mdd");
    if (champMdd) champMdd.textContent = `${(champ.Max_Drawdown_Pct ?? 16.4).toFixed(1)}%`;
    const champSh = document.getElementById("champ-sharpe");
    if (champSh) champSh.textContent = (champ.Sharpe ?? 2.08).toFixed(2);
    const champPf = document.getElementById("champ-pf");
    if (champPf) champPf.textContent = (champ.Profit_Factor ?? 4.64).toFixed(2);
    const champFee = document.getElementById("champ-fee");
    if (champFee) champFee.textContent = `${(champ.Fees_Applied_Pct ?? 1.5).toFixed(1)}%`;
  }
...
function openRiskDrillDown(strat) {
  ...
  const riskFeePct = strat.Fees_Applied_Pct ?? ((strat.Total_Trades ?? 16) * 0.1);
  document.getElementById("modal-fees").textContent = `${riskFeePct.toFixed(1)}%`;
  const riskFeeUsd = ((strat.Final_Equity ?? 10000) * riskFeePct / 100);
  document.getElementById("modal-fees-usd").textContent = `-$${riskFeeUsd.toFixed(2)} friction`;

  document.getElementById("modal-composite-score").textContent = `${(strat.Composite_Score ?? ((strat.Sharpe ?? 1.5) * 20 + (strat.Calmar ?? 3) * 10)).toFixed(1)} / 100`;
  const posMths = [strat.M_Jan, strat.M_Feb, strat.M_Mar, strat.M_Apr, strat.M_May, strat.M_Jun, strat.M_Jul, strat.M_Aug].filter(v => v != null && v > 0).length;
  document.getElementById("modal-pos-months").textContent = `${posMths > 0 ? posMths : '—'}/8 Pos Months`;
