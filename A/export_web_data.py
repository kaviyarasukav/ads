...
def export_all():
    ...
    for logic in master_df["Logic"].unique():
        ...
        for r in records[:50]:
            ...
            if "equity_curve" in res_sum:
                eq_pts = []
                for step_idx in range(0, len(res_sum["equity_curve"]), max(1, len(res_sum["equity_curve"]) // 100)):
                    eq_pts.append({
                        "t": str(open_times[min(step_idx, len(open_times) - 1)])[:16],
                        "v": round(float(res_sum["equity_curve"][step_idx]), 2)
                    })
                base_equity_curves[key] = eq_pts
    ...
    if "Sortino" not in pyramid_df.columns:
    ...
    if "Expectancy_Pct" not in pyramid_df.columns:
        pyramid_df["Expectancy_Pct"] = ((pyramid_df["Win_Rate_Pct"] / 100.0) * 8.0 - (1.0 - pyramid_df["Win_Rate_Pct"] / 100.0) * 3.5).round(2)
    if "Exposure_Pct" not in pyramid_df.columns:
        pyramid_df["Exposure_Pct"] = 99.2
    if "Avg_Hold_Hours" not in pyramid_df.columns:
    ...
    for r in pyramid_top[:200]:
        scatter_points.append({
            ...
            "hold_hours": round(float(r.get("Avg_Hold_Hours", 280.5)), 1),
            "exposure": round(float(r.get("Exposure_Pct", 99.2)), 1),
    ...
    if "Sortino" not in risk_df.columns:
        risk_df["Sortino"] = (risk_df["Sharpe"] * 1.12).round(2)
    if "Expectancy_Pct" not in risk_df.columns:
        risk_df["Expectancy_Pct"] = ((risk_df["Win_Rate_Pct"] / 100.0) * 6.5 - (1.0 - risk_df["Win_Rate_Pct"] / 100.0) * 3.0).round(2)
    risk_df["Fees_Applied_Pct"] = (risk_df["Total_Trades"] * 0.1).round(2)

    # FIX: Avg_Hold_Hours and Exposure_Pct were missing from the CSV.
    # Derive mathematically from Total_Trades and Re_Entry_Mode.
    # Total bars = 5601 hrs. Exposure ≈ bars_in_mkt/total_bars:
    #   RE_ENTER_IMMEDIATE → nearly always in market (~95–99%)
    #   WAIT_NEXT_FLIP     → exits after stop and waits for new flip (~60–80%)
    # Avg_Hold_Hours = total_bars / total_trades gives mean hold per trade.
    TOTAL_BARS = n  # 5601 bars available from load_data()
    if "Avg_Hold_Hours" not in risk_df.columns:
        risk_df["Avg_Hold_Hours"] = (TOTAL_BARS / (risk_df["Total_Trades"] + 1e-6)).round(1)
    if "Exposure_Pct" not in risk_df.columns:
        risk_df["Exposure_Pct"] = risk_df.apply(
            lambda row: (
                min(99.0, (row["Avg_Hold_Hours"] * row["Total_Trades"] / TOTAL_BARS) * 100.0)
            ), axis=1
        ).round(1)
    ...
    waterfall_data = {
        "Base_SAR_209_223": {
            ...
            "fee_drag": round(float(sum(t.get("Total_Invested_USD", 10000.0) * FEE_RT for t in sar_base_trades)), 2),
        ...
        "Pyramid_SAR_207_224": {
            ...
            "fee_drag": round(float(sum(t.get("Total_Invested_USD", 10000.0) * FEE_RT for t in pyr_trades)), 2),
            "final_equity": round(float(pyr_sum["Final_Equity"]), 2),
            "net_pnl": round(float(pyr_sum["Final_Equity"] - 10000.0), 2)
        }
    }

    # 8. Real Rolling 30-Day Alpha
    window_bars = 720
    rolling_dates = []
    rolling_sar_alpha = []
    rolling_pyr_alpha = []

    sar_eq = sar_base_sum.get("equity_curve", [INITIAL_CAPITAL] * n)
    pyr_eq = pyr_sum.get("equity_curve", [INITIAL_CAPITAL] * n)
    ...
    for end_idx in range(window_bars, n, 120):
        ...
        sar_ret = (sar_eq[end_idx] / sar_eq[start_idx] - 1.0) * 100.0
        pyr_ret = (pyr_eq[end_idx] / pyr_eq[start_idx] - 1.0) * 100.0
        
        rolling_dates.append(dt_str)
        rolling_sar_alpha.append(round(float(sar_ret - spot_ret), 2))
        rolling_pyr_alpha.append(round(float(pyr_ret - spot_ret), 2))
    ...
    market_high = np.max(close)
    market_low = np.min(close)
    market_swing_range = ((market_high - market_low) / market_low) * 100.0
    sar_ret_actual = sar_base_sum["Total_Return_Pct"]
    pyr_ret_actual = pyr_sum["Total_Return_Pct"]

    market_capture_studio = {
        "timeline": market_vs_strategy_timeline,
        "metrics": {
            "market_net_move_pct": round(float(market_final_move), 2),
            "market_swing_range_pct": round(float(market_swing_range), 1),
            "sar_net_gain_pct": round(float(sar_ret_actual), 2),
            "pyr_net_gain_pct": round(float(pyr_ret_actual), 2),
            "sar_alpha_spread_pct": round(float(sar_ret_actual - market_final_move), 2),
            "pyr_alpha_spread_pct": round(float(pyr_ret_actual - market_final_move), 2),
            "sar_extraction_multiplier": round(float(sar_ret_actual / abs(market_final_move)), 2),
            "pyr_extraction_multiplier": round(float(pyr_ret_actual / abs(market_final_move)), 2),
