...
def run_risk_trade_simulation(
    ...
    for i in range(1, n):
        ...
        # Entry logic (only if not already in position, and did not just exit in this same bar)
        elif not in_pos and curr_sig != 0:
            allow_entry = False
    ...
    avg_win_p = float(np.mean(win_trades)) if win_trades else 0.0
    avg_loss_p = abs(float(np.mean(loss_trades))) if loss_trades else 0.0
    wr_f = win_rate / 100.0
